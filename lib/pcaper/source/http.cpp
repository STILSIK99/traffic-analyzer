...
Http::Http (const u_char *pkt_data, int size): begin (pkt_data), len (size)
{
}

Http::Http() {}

QJsonObject Http::writeReport (std::set<QString> &attributes)
{
    QJsonObject http;
    int s_len = 0;
...
void Http::foundKey (std::pair<bool, QString> &pair, const u_char *begin, int len)
{
    if (len == 12) {
...
void Http::getValue (QString &key, const u_char *begin, int len)
{
    key = QString (len, 0);
    for (int i = 0; i < len; ++i) {
        key[i] = begin[i];
...
bool Http::checkApplication (const u_char *pkt, int appLen)
{
    std::string app (appLen, ' ');
...
std::pair<QString, QString> Http::getToken (QString str)
{
    std::string token = QByteArray::fromBase64 (str.toUtf8()).toStdString();
